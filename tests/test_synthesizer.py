...
async def test_generateLevelForUser():
    async with AsyncSessionLocal() as session:
        test_user_id = uuid.uuid4()
        level_response = await generateLevelForUser(session, userId=test_user_id, requestedDifficulty=0.35)
        ...
        for clue in level_response.clues:
            print(f"  • [{clue['direction']}] {clue['display_title']}: {clue['clue_text'][:80]}...")


async def test_played_movie_deduplication():
    async with AsyncSessionLocal() as session:
        test_user_id = uuid.uuid4()
        ...
        # 1. Generate Level 1
        level1 = await generateLevelForUser(session, userId=test_user_id, requestedDifficulty=0.35)
        level1_imdb_ids = {c["imdb_id"] for c in level1.clues if c["imdb_id"]}
        assert len(level1_imdb_ids) > 0

        # 2. Record Level 1 movies as played in UserGameplayTelemetry
        ...
        level2 = await generateLevelForUser(session, userId=test_user_id, requestedDifficulty=0.35)
